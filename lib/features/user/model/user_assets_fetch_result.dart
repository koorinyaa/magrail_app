...
final class UserAssetsFetchResult {
  ...
  const UserAssetsFetchResult.success(UserDetailProfile profile)
    : this(status: UserAssetsFetchStatus.success, profile: profile);

  /// 创建会话失效结果
  ///
  /// [message] 结果消息
  const UserAssetsFetchResult.authExpired(String message)
    : this(status: UserAssetsFetchStatus.authExpired, message: message);

  /// 创建失败结果
  ///
  /// [message] 结果消息
  const UserAssetsFetchResult.failure(String message)
    : this(status: UserAssetsFetchStatus.failure, message: message);
