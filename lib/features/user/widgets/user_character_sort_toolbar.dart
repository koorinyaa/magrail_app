...
/// 用户角色排序工具栏
class UserCharacterSortToolbar extends StatelessWidget
  ...
  const UserCharacterSortToolbar({
    super.key,
  ...
  /// 排序选择回调
  final ValueChanged<UserCharacterSnapshotSort> onSortSelected;
          ...
          builder: (context, child) {
            ...
            return Align(
              ...
              child: SingleChildScrollView(
                ...
                child: Row(
                  children: [
                    for (final option in UserCharacterSnapshotSort.values.where(
                      (option) =>
                          isCurrentUser ||
                          option != UserCharacterSnapshotSort.totalDividend,
                    )) ...[
                      if (option != UserCharacterSnapshotSort.values.first)
                        const SizedBox(width: 6),
                      _UserCharacterSortChip(
                        label: option.label,
...
/// 用户角色排序选择项
class _UserCharacterSortChip extends StatelessWidget {
  /// 创建用户角色排序选择项
  ///
  /// [label] 选择项文案
  /// [isSelected] 是否选中
  ...
  /// [onPressed] 点击回调，为空时禁止切换
  const _UserCharacterSortChip({
    required this.label,
  ...
  final UserCharacterSnapshotSortDirection direction;
